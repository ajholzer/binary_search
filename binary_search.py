...
def find_smallest_positive(xs):
    ...
    def go(start, end):
        ...
        if start == end:
            if xs[start] == 0:
                return xs.index(xs[start + 1])
            elif xs[start] < 0:
                return None
            else:
                return xs.index(xs[start])
        if xs[mid] < 0:
            start = mid + 1
        if xs[mid] > 0:
            end = mid
        if xs[mid] == 0:
            return xs.index(xs[mid+1])
        return go(start, end)
...
def argmin(f, lo, hi, epsilon=1e-3):
    '''
    Assumes that f is an input function
    that takes a float as input and returns
    a float with a unique global minimum,
    and that lo and hi are both floats satisfying lo < hi.
    Returns a number that is within epsilon
    of the value that minimizes f(x) over the interval [lo,hi]
    HINT:
    The basic algorithm is:
        ...
        2) For each recursive call:
            ...
            b) one of the 4 points (lo,m1,m2,hi) must be the smallest;
               ...
               you recursively call your
               function on the interval [lo,m2] or [m1,hi]
    ...
    about different f functions that can
    be minimized and their applications.
    But the actual minimization code will a
    ll be a variant of this binary search.

    WARNING:
    The doctests below are not intended to pass on your code,
    and are only given so that you have
    an example of what the output should look like.
    Your output numbers are likely to
    be slightly different due to minor implementation details.
    Writing tests for code that u
    ses floating point numbers is notoriously difficult.
